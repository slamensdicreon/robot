/** Local-dev backend: better-sqlite3 (synchronous), wrapped in async fns. */
...
export async function updateAccountStatus(id, status, fields = {}) {
  ...
  d.prepare(`UPDATE accounts SET ${sets.join(', ')} WHERE id = @id`).run(params);
}

export async function saveAssessment(accountId, a) {
  const d = await getDb();
  d.prepare('DELETE FROM assessments WHERE account_id = ?').run(accountId);
  d.prepare(`
...
export async function getAssessment(accountId) {
  const d = await getDb();
  return d.prepare('SELECT * FROM assessments WHERE account_id = ?').get(accountId);
}

export async function findDuplicate(sessionId, name) {
  const d = await getDb();
  return d.prepare('SELECT id FROM accounts WHERE session_id = ? AND lower(name) = lower(?)')
    .get(sessionId, name);
}
